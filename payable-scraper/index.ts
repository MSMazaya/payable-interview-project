...
//             let result = []
//
//             const nextButtonExists = await this.page.$('#Action\\.OpTransactionListing_custom\\.GOTO_NEXT__');
...
//
//                 if (nextButtonExists) {
//                     nextButtonClickable = await this.page.$eval('#Action\\.OpTransactionListing_custom\\.GOTO_NEXT__', el => !el.getAttributeNames().includes("disabled"))
//                     console.log('nextButtonClickable ', nextButtonClickable)
//                     if (nextButtonClickable) {
//                         await this.page.click('#Action\\.OpTransactionListing_custom\\.GOTO_NEXT__')
//                         await this.page.waitForNavigation()
//                     }
//                 } else {
...
const getData = async () => {
    const builder = new ScraperBuilder().pickEngine('puppeteer').pickScraper('BNI').setCreds({ accountId: process.env.ACCOUNT, password: process.env.PASSWORD }).build;
    const scraper = await builder()

    scraper.login().then(() => scraper.getTransactions(moment('20-10-2022', 'DD-MM-YYYY').toDate(), new Date()).then(() => scraper.logout()))
}
